...
export function ClientPicker({ variant, onChange }: Props) {
  ...
  if (variant === 'onboarding') {
    return (
      <div className="rounded-xl bg-pi-surface/90 backdrop-blur p-6 border border-pi-border max-w-md w-full">
        <h3 className="text-lg font-medium mb-1">Para quem é este post?</h3>
        <p className="text-sm text-pi-text-muted mb-4">
          Quanto mais posts este cliente tem, mais o Diretor IA aprende.
        </p>
        {loading ? (
          <div className="flex items-center gap-2 text-sm text-pi-text-muted">
          ...
          <div className="flex flex-wrap gap-2">
            {clients.map(c => (
              <button
                key={c.id}
                onClick={() => select(c.id)}
                className="flex items-center gap-2 px-3 py-2 rounded-lg bg-pi-surface-muted hover:bg-pi-border transition"
              >
            ...
            <a
              href="/clients"
              className="flex items-center gap-2 px-3 py-2 rounded-lg bg-pi-surface-muted hover:bg-pi-border transition text-pi-accent text-sm"
            >
              + Novo cliente
            </a>
          </div>
    ...
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={`flex items-center gap-2 rounded-md transition
          ${isHeader ? 'px-3 py-1.5 bg-pi-surface-muted/80 hover:bg-pi-surface-muted' : 'px-2 py-1 bg-pi-surface-muted/40 hover:bg-pi-surface-muted/70'}
          ${!selected ? 'ring-2 ring-pi-accent animate-pulse' : ''}`}
      >
        {loading ? (
          <span className="text-sm text-pi-text-muted">Carregando…</span>
        ) : selected ? (
          <>
          ...
          <span className="text-sm text-pi-text-muted">Selecione o cliente</span>
        )}
        <ChevronDown className="h-4 w-4 opacity-60" />
      </button>

      {open && (
        <div className="absolute top-full left-0 mt-1 w-72 rounded-lg bg-pi-surface border border-pi-border shadow-xl z-50 max-h-80 overflow-auto">
          {clients.length === 0 ? (
            <p className="text-sm text-pi-text-muted px-3 py-4 text-center">Nenhum cliente cadastrado</p>
          ) : clients.map(c => (
            <button
              key={c.id}
              onClick={() => select(c.id)}
              className="w-full flex items-center gap-3 px-3 py-2 hover:bg-pi-surface-muted text-left"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-pi-text-muted">
                  {c.postCount} posts
                  {c.hasDnaVisual
                    ? ` • DNA ${c.dnaConfidence}%`
                    : ' • DNA pendente'}
          ...
          <div className="border-t border-pi-border">
            <a
              href="/clients"
              className="block px-3 py-2 text-sm text-pi-accent hover:bg-pi-surface-muted"
            >
              + Gerenciar clientes
            </a>
          </div>
