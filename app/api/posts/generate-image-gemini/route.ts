...
function buildGeminiPrompt(
  ...
  return [
    `Create a professional Instagram post image (4:5 portrait, 1080×1350px).`,
    ``,
    `VISUAL CONCEPT: ${visual || "High-quality brand photography, clean and modern."}`,
    ``,
    `TEXT TO EMBED IN THE IMAGE (mandatory — place prominently):`,
    `"${headline}"`,
    ``,
    `BRAND CONTEXT:`,
    ...
    `- Tone: ${(client.tone_of_voice ?? "").slice(0, 120)}`,
    brandDNA,
    ``,
    `DESIGN RULES:`,
    `- The headline text must be legible, bold, large — integrated into the image design`,
    `- Leave the bottom strip (about 5% of height) completely empty — brand footer will be composited later`,
    `- Leave the top-left corner completely blank with no text, no graphics, no decorations — a brand logo will be placed there in post-processing`,
    `- High contrast text against background`,
    `- Do NOT add any logo or watermark — compositor handles this`,
    `- Output: single composed image, no borders, no padding`,
  ].join("\n");
}
