...
async function getDnaVisual(uid: string, clientId: string): Promise<BrandDNA | null> {
  ...
  const legacy = await adminDb.collection('clients').doc(clientId).collection('brand_dna').doc('current').get();
  return legacy.exists ? (legacy.data() as BrandDNA) : null;
}

async function getRecentApprovedPosts(uid: string, clientId: string, limit: number): Promise<{ primary_color?: string; secondary_color?: string }[]> {
...
function countColorsFromPosts(posts: { primary_color?: string; secondary_color?: string }[]): Map<string, number> {
...
export async function computeLockSuggestions(uid: string, clientId: string): Promise<LockSuggestion[]> {
  ...
  if (dna) {
    ...
    const typoPattern = dna.typography_pattern;
    if (typoPattern) {
      suggestions.push({
        lock: {
          scope: 'typography',
        ...
        source: 'dna_visual',
        confidence: dna.confidence_score / 100,
      });
    }
